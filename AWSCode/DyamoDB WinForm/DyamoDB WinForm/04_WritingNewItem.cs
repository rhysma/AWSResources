  ...
  public static partial class Ddb_Intro
  {
    ...
    public static async Task WritingNewMovie_async( Document newItem )
    {
      ...
      else
      {
        ...
        public static async Task WritingNewItem_async(Document newItem, Table openTable)
        {
            operationSucceeded = false;
            operationFailed = false;

            int id = (int)newItem["ID"];
            string name = newItem["Name"];


              try
                {
                    Task<Document> writeNew = openTable.PutItemAsync(newItem);
                    Console.WriteLine("  -- Writing a new item to the table...");
                    await writeNew;
                    Console.WriteLine("      -- Wrote the item successfully!");
                    operationSucceeded = true;
                }
                ...
                {
                    Console.WriteLine("      FAILED to write the new item, because:\n       {0}.", ex.Message);
                    operationFailed = true;
                }

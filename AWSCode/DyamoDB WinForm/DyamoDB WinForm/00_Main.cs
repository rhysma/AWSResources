...
namespace DyamoDB_WinForm
{
  public static partial class Ddb_Intro
  {
    ...
    public static Table moviesTable;
        public static Table openTableName;
     ...
     *--------------------------------------------------------------------------*/
        ...
        {
            Console.WriteLine("Create a DynamoDB client connected to a DynamoDB-Local instance");
        ...
        public static int CreateTable(string tableName, List<AttributeDefinition> attr, List<KeySchemaElement> schema)
        {
            //  2.  Create a table for movie data asynchronously
            Console.WriteLine("Create a table");
            CreatingTable_async(tableName, attr, schema,
                                 provisioned_throughput);
            if (!pause() || operationFailed)
            {
                Console.WriteLine("error in creating table");
                return 1;
            }
                
            try { openTableName = Table.LoadTable(Ddb_Intro.client, tableName); }
            catch (Exception ex)
            {
                operationFailed = true;
                Console.WriteLine(
                  " Error: Could not access the new '{0}' table after creating it;\n" +
                  "        Reason: {1}.", tableName, ex.Message);
                pause();
                return 1;
            }
            return 0;
        ...
        public static int AddItem(int id, string name, string tableName)
        {
            Console.WriteLine("Add a new item to the table");
            Document newItemDocument = new Document();
            newItemDocument["ID"] = id;
            newItemDocument["Name"] = name;

            try 
            { 
                
                openTableName = Table.LoadTable(Ddb_Intro.client, tableName);

                WritingNewItem_async(newItemDocument, openTableName);
                if (!pause() || operationFailed)
                {
                    return (1);
                }
            ...
            catch (Exception ex)
            {
                operationFailed = true;
                Console.WriteLine(
                  " Error: Could not access the table Reason: {0}.", ex.Message);
                pause();
                return 1;
            }
        ...
        public static List<Person> GetRecords(string tableName)
        {
            List<Person> records = new List<Person>();

            using(IAmazonDynamoDB client = Ddb_Intro.client)
            {
                Table openTable = Table.LoadTable(client, tableName);

                //get all records
                ScanFilter scanFilter = new ScanFilter();
                Search getAllItems = openTable.Scan(scanFilter);
                //if there are no records to get this is going to thrown an exception so it's best to check first
                ...
                foreach(Document item in allItems)
                {
                    ...
                    foreach (string key in item.Keys)
                    {
                        ...
                        if(key.ToLower() == "id")
                        {
                            myPerson.ID = Convert.ToInt32(val);
                        }
                        if(key.ToLower() == "name")
                        {
                            myPerson.Name = val;
    ...
    public static int DatabaseFunctions( )
    {
